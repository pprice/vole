//! Rule: tautological when expression.
//!
//! Generates when expressions with always-true conditions:
//! `x == x`, `x >= x`, `x * 0 == 0`, etc.
...
pub struct TautologicalWhen;

impl StmtRule for TautologicalWhen {
    ...
    fn generate(&self, scope: &mut Scope, emit: &mut Emit, _params: &Params) -> Option<String> {
        ...
        let cond = match emit.gen_range(0..4) {
            0 => format!("{} == {}", var, var),
            1 => format!("{} >= {}", var, var),
            2 => format!("{} <= {}", var, var),
            _ => format!("{} * 0 == 0", var),
...
mod tests {
    ...
    #[test]
    fn name_is_correct() {
        assert_eq!(TautologicalWhen.name(), "tautological_when");
    }
    ...
    fn generates_when() {
        ...
        let result = TautologicalWhen.generate(&mut scope, &mut emit, &params);
