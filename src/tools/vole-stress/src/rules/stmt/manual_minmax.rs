//! Rule: manual min/max via when expression.
//!
//! Generates `when { a > b => a, _ => b }` (max) or
//! `when { a < b => a, _ => b }` (min).
...
impl StmtRule for ManualMinmax {
    ...
    fn generate(&self, scope: &mut Scope, emit: &mut Emit, _params: &Params) -> Option<String> {
        ...
        let a_idx = emit.gen_range(0..i64_vars.len());
        let a = &i64_vars[a_idx];
        let mut b_idx = emit.gen_range(0..i64_vars.len());
        while b_idx == a_idx && i64_vars.len() > 1 {
            b_idx = emit.gen_range(0..i64_vars.len());
        }
        let b = &i64_vars[b_idx];
...
mod tests {
    ...
    #[test]
    fn name_is_correct() {
        assert_eq!(ManualMinmax.name(), "manual_minmax");
    }
    ...
    fn generates_minmax() {
        ...
        let result = ManualMinmax.generate(&mut scope, &mut emit, &params);
