//! Rule: nested `.to_string().length().to_string()` chain.
//!
//! Generates chained method calls on numeric variables like
//! `x.to_string().length().to_string()` or `x.to_string().contains("0")`.
...
pub struct NestedTostring;

impl StmtRule for NestedTostring {
...
mod tests {
    ...
    #[test]
    fn name_is_correct() {
        assert_eq!(NestedTostring.name(), "nested_tostring");
    }
    ...
    fn generates_chain() {
        ...
        let result = NestedTostring.generate(&mut scope, &mut emit, &params);
