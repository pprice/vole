//! Rule: match with `.to_string()` in wildcard arm.
//!
//! Generates `match x { 0 => "0", 1 => "1", _ => x.to_string() }`.
...
impl StmtRule for MatchTostringArms {
    ...
    fn generate(&self, scope: &mut Scope, emit: &mut Emit, _params: &Params) -> Option<String> {
        ...
        let num_arms = emit.random_in(1, 3);

        let mut arms = String::new();
        let mut used = std::collections::HashSet::new();
        for _ in 0..num_arms {
            let val = emit.gen_range(0..10) as i64;
            if used.insert(val) {
...
mod tests {
    ...
    #[test]
    fn name_is_correct() {
        assert_eq!(MatchTostringArms.name(), "match_tostring_arms");
    }
