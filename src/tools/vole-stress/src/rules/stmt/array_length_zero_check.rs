//! Rule: safe first-element access with length guard.
//!
//! Generates `when { arr.length() > 0 => arr[0], _ => default }`.
//! Similar to array_length_guard but with different default patterns.
...
pub struct ArrayLengthZeroCheck;

impl StmtRule for ArrayLengthZeroCheck {
    fn name(&self) -> &'static str {
        "array_length_zero_check"
    }
    ...
    fn generate(&self, scope: &mut Scope, emit: &mut Emit, _params: &Params) -> Option<String> {
        ...
        scope.add_local(name.clone(), TypeInfo::Primitive(*elem_type), false);
        Some(format!(
...
mod tests {
    ...
    #[test]
    fn name_is_correct() {
        assert_eq!(ArrayLengthZeroCheck.name(), "array_length_zero_check");
    }
    ...
    fn generates_check() {
        ...
        let result = ArrayLengthZeroCheck.generate(&mut scope, &mut emit, &rule_params);
