...
impl TypeId {
    ...
    pub fn from_type(ty: &Type, types: &TypeTable) -> Option<Self> {
        match ty {
            ...
            Type::Iterator(_) => types.iterator_name_id().map(TypeId),
            Type::MapIterator(_) => types.map_iterator_name_id().map(TypeId),
            Type::FilterIterator(_) => types.filter_iterator_name_id().map(TypeId),
            Type::TakeIterator(_) => types.take_iterator_name_id().map(TypeId),
            Type::SkipIterator(_) => types.skip_iterator_name_id().map(TypeId),
