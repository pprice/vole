        ...
        if let Some(type_id) = TypeId::from_type(&target_type, &self.type_table) {
            ...
            if let Some(ref statics_block) = impl_block.statics {
                ...
                let entity_type_id = match &target_type {
                    ...
                    Type::I128 => {
                        self.entity_registry.type_by_name(self.name_table.primitives.i128)
                    }
                    ...
                    Type::Bool => {
                        self.entity_registry.type_by_name(self.name_table.primitives.bool)
                    }
                    Type::String => {
                        self.entity_registry.type_by_name(self.name_table.primitives.string)
                    }
                    _ => None,
                };
