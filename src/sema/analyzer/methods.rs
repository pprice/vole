...
impl Analyzer {
    ...
    pub(crate) fn get_functional_interface_type(
        ...
        let method = self.interface_registry.is_functional(interface_name, interner)?;
        Some(FunctionType {
    ...
    pub fn satisfies_interface(&self, ty: &Type, interface_name: Symbol, interner: &Interner) -> bool {
    ...
    pub(crate) fn type_has_field(
        ...
        match ty {
            Type::Record(r) => r
                .fields
                .iter()
                .any(|f| f.name == field_name && self.types_compatible(&f.ty, expected_type, interner)),
            Type::Class(c) => c
                .fields
                .iter()
                .any(|f| f.name == field_name && self.types_compatible(&f.ty, expected_type, interner)),
