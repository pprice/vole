...
impl<'a> Compiler<'a> {
    ...
    fn register_class(&mut self, class: &ClassDecl, program: &Program) {
        ...
        for (i, field) in class.fields.iter().enumerate() {
            ...
            struct_fields.push(StructField {
                name: field.name,
                ty: resolve_type_expr_full(&field.ty, &self.type_aliases, &self.interface_registry, self.interner),
                slot: i,
            });
        }
        ...
        for method in &class.methods {
            let return_type = method
                .return_type
                .as_ref()
                .map(|t| resolve_type_expr_full(t, &self.type_aliases, &self.interface_registry, self.interner))
                .unwrap_or(Type::Void);
            ...
            && let Some(metadata) = self.type_metadata.get_mut(&type_sym)
        {
            for method in &impl_block.methods {
                let return_type = method
                    ...
                    .as_ref()
                    .map(|t| {
                        resolve_type_expr_full(t, &self.type_aliases, &self.interface_registry, self.interner)
                    })
                    .unwrap_or(Type::Void);
                metadata
                    .method_return_types
    ...
    fn compile_implement_method(
        ...
        let param_types: Vec<types::Type> = method
            ...
            .map(|p| {
                type_to_cranelift(
                    &resolve_type_expr_full(&p.ty, &self.type_aliases, &self.interface_registry, self.interner),
                    self.pointer_type,
                )
            })
            .collect();
        let param_vole_types: Vec<Type> = method
            .params
            .iter()
            .map(|p| resolve_type_expr_full(&p.ty, &self.type_aliases, &self.interface_registry, self.interner))
            .collect();
