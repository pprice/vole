...
// Re-export Symbol from vole-identity (canonical definition)
pub use vole_identity::Symbol;
...
// Re-export PrimitiveType from vole-identity (canonical definition)
pub use vole_identity::PrimitiveType;
...
// ---------------------------------------------------------------------------
// Iterative Drop for Program
//
// The recursive destructor for deeply nested AST trees is expensive — each
// `Box<Expr>` triggers a recursive call with function prologue/epilogue
// overhead. `Program::drop` uses an explicit work stack to linearise
// destruction: it walks all declarations, extracts every `ExprKind` and
// `TypeExprKind`, and drops them iteratively. The compiler-generated
// destructors then only see shallow sentinel values.
// ---------------------------------------------------------------------------

/// Work items for the iterative drop loop.
enum DropItem {
    Expr(ExprKind),
    Type(TypeExprKind),
}
...
impl Drop for Program {
    fn drop(&mut self) {
        ...
        for decl in std::mem::take(&mut self.declarations) {
            drain_decl(&mut stack, decl);
        }
        while let Some(item) = stack.pop() {
            match item {
                DropItem::Expr(kind) => drain_expr_kind(&mut stack, kind),
                DropItem::Type(kind) => drain_type_expr_kind(&mut stack, kind),
            }
        }
    }
}

/// Extract all child `Expr`/`TypeExpr` nodes from a `Decl`.
fn drain_decl(stack: &mut Vec<DropItem>, decl: Decl) {
    match decl {
        Decl::Function(func) => drain_func_decl(stack, func),
        Decl::Tests(tests) => {
            for d in tests.decls {
                drain_decl(stack, d);
            }
            for tc in tests.tests {
                drain_func_body(stack, tc.body);
            }
        }
        Decl::Let(let_stmt) => drain_let_stmt(stack, let_stmt),
        Decl::LetTuple(lt) => {
            drain_pattern(stack, lt.pattern);
            take_expr(stack, lt.init);
        }
        Decl::Class(cls) => {
            drain_type_params(stack, cls.type_params);
            for iface in cls.implements {
                take_type_expr(stack, iface);
            }
            for f in cls.fields {
                drain_field_def(stack, f);
            }
            for m in cls.methods {
                drain_func_decl(stack, m);
            }
            if let Some(statics) = cls.statics {
                drain_statics_block(stack, statics);
            }
        }
        Decl::Struct(s) => {
            drain_type_params(stack, s.type_params);
            for f in s.fields {
                drain_field_def(stack, f);
            }
            for m in s.methods {
            ...
            if let Some(statics) = s.statics {
                drain_statics_block(stack, statics);
            }
        }
        Decl::Interface(iface) => {
            drain_type_params(stack, iface.type_params);
            for f in iface.fields {
                drain_field_def(stack, f);
            }
            for m in iface.methods {
                drain_interface_method(stack, m);
            }
        ...
        Decl::Implement(imp) => {
            if let Some(tt) = imp.trait_type {
                take_type_expr(stack, tt);
            }
            take_type_expr(stack, imp.target_type);
            for m in imp.methods {
                drain_func_decl(stack, m);
            }
            if let Some(statics) = imp.statics {
                drain_statics_block(stack, statics);
            }
        }
        Decl::Error(err) => {
        ...
        Decl::Sentinel(_) => {}
        Decl::External(ext) => drain_external_block(stack, ext),
    }
}

/// Drain a `FuncDecl`.
fn drain_func_decl(stack: &mut Vec<DropItem>, func: FuncDecl) {
    drain_type_params(stack, func.type_params);
    for p in func.params {
        drain_param(stack, p);
    }
    if let Some(rt) = func.return_type {
        take_type_expr(stack, rt);
    }
    drain_func_body(stack, func.body);
}

/// Drain a `Param`.
fn drain_param(stack: &mut Vec<DropItem>, param: Param) {
    take_type_expr(stack, param.ty);
    if let Some(dv) = param.default_value {
        take_boxed_expr(stack, dv);
    }
}

/// Drain a `FieldDef`.
fn drain_field_def(stack: &mut Vec<DropItem>, field: FieldDef) {
    take_type_expr(stack, field.ty);
    if let Some(dv) = field.default_value {
        take_boxed_expr(stack, dv);
    }
}

/// Drain an `InterfaceMethod`.
fn drain_interface_method(stack: &mut Vec<DropItem>, method: InterfaceMethod) {
    drain_type_params(stack, method.type_params);
    ...
    if let Some(body) = method.body {
        drain_func_body(stack, body);
    }
}

/// Drain a `StaticsBlock`.
fn drain_statics_block(stack: &mut Vec<DropItem>, statics: StaticsBlock) {
    for m in statics.methods {
        drain_interface_method(stack, m);
    }
    for ext in statics.external_blocks {
        drain_external_block(stack, ext);
    }
}

/// Drain an `ExternalBlock`.
fn drain_external_block(stack: &mut Vec<DropItem>, ext: ExternalBlock) {
    for f in ext.functions {
        drain_type_params(stack, f.type_params);
        for p in f.params {
            drain_param(stack, p);
        }
        ...
        if let Some(mappings) = f.type_mappings {
            for mapping in mappings {
                if let TypeMappingArm::Exact(te) = mapping.arm {
                    take_type_expr(stack, te);
                }
            }
        }
    }
...
/// Drain a `LetStmt`.
fn drain_let_stmt(stack: &mut Vec<DropItem>, let_stmt: LetStmt) {
    if let Some(te) = let_stmt.ty {
        take_type_expr(stack, te);
    }
    match let_stmt.init {
        LetInit::Expr(expr) => take_expr(stack, expr),
        LetInit::TypeAlias(te) => take_type_expr(stack, te),
    }
}

/// Push an `ExprKind` onto the work stack after replacing the `Expr`'s kind
/// with a cheap sentinel (`Unreachable`). The remaining `Expr` fields
/// (`NodeId`, `Span`) are `Copy`, so nothing is leaked.
#[inline]
fn take_expr(stack: &mut Vec<DropItem>, mut expr: Expr) {
    let kind = std::mem::replace(&mut expr.kind, ExprKind::Unreachable);
    stack.push(DropItem::Expr(kind));
}

/// Same as `take_expr` but for a boxed expression.
/// Callers already hold `Box<Expr>` from AST fields; accepting the box
/// directly avoids an unnecessary intermediate move.
#[allow(clippy::boxed_local)]
#[inline]
fn take_boxed_expr(stack: &mut Vec<DropItem>, expr: Box<Expr>) {
    take_expr(stack, *expr);
}

/// Push a `TypeExprKind` onto the work stack after replacing the `TypeExpr`'s
/// kind with a cheap sentinel (`Handle`).
#[inline]
fn take_type_expr(stack: &mut Vec<DropItem>, mut te: TypeExpr) {
    let kind = std::mem::replace(&mut te.kind, TypeExprKind::Handle);
    stack.push(DropItem::Type(kind));
}

/// Same as `take_type_expr` but for a boxed `TypeExpr`.
#[allow(clippy::boxed_local)]
#[inline]
fn take_boxed_type_expr(stack: &mut Vec<DropItem>, te: Box<TypeExpr>) {
    take_type_expr(stack, *te);
}

/// Extract child `Expr` values from an `AssignTarget` and push onto stack.
fn drain_assign_target(stack: &mut Vec<DropItem>, target: AssignTarget) {
    match target {
        AssignTarget::Variable(_) | AssignTarget::Discard => {}
        AssignTarget::Index { object, index } => {
            take_boxed_expr(stack, object);
            take_boxed_expr(stack, index);
        }
...
/// Extract child `Expr` and `TypeExpr` values from a `Pattern`.
fn drain_pattern(stack: &mut Vec<DropItem>, pattern: Pattern) {
    let mut pat_stack = vec![pattern.kind];
    while let Some(pk) = pat_stack.pop() {
        match pk {
            PatternKind::Wildcard | PatternKind::Identifier { .. } | PatternKind::Val { .. } => {}
            PatternKind::Literal(expr) => {
                take_expr(stack, expr);
            }
            PatternKind::Type { type_expr } => {
                take_type_expr(stack, type_expr);
            }
            PatternKind::Success { inner } | PatternKind::Error { inner } => {
                if let Some(p) = inner {
                    pat_stack.push(p.kind);
                }
            }
...
/// Extract children from a `Stmt`.
fn drain_stmt(stack: &mut Vec<DropItem>, stmt: Stmt) {
    match stmt {
        Stmt::Let(let_stmt) => drain_let_stmt(stack, let_stmt),
        Stmt::LetTuple(lt) => {
            drain_pattern(stack, lt.pattern);
            take_expr(stack, lt.init);
        }
        ...
        Stmt::While(WhileStmt {
            ...
            take_expr(stack, condition);
            drain_block(stack, body);
        }
        Stmt::For(ForStmt { iterable, body, .. }) => {
            take_expr(stack, iterable);
            drain_block(stack, body);
        }
        Stmt::If(IfStmt {
            condition,
        ...
        Stmt::Raise(RaiseStmt { fields, .. }) => {
            for f in fields {
                take_expr(stack, f.value);
            }
        }
    }
}
...
fn drain_block(stack: &mut Vec<DropItem>, block: Block) {
    for stmt in block.stmts {
        drain_stmt(stack, stmt);
    }
}

/// Extract children from a `FuncBody`.
fn drain_func_body(stack: &mut Vec<DropItem>, body: FuncBody) {
    match body {
        FuncBody::Block(block) => drain_block(stack, block),
        FuncBody::Expr(expr) => take_boxed_expr(stack, expr),
    }
}

/// Extract all child nodes from an `ExprKind` and push them onto the stack.
fn drain_expr_kind(stack: &mut Vec<DropItem>, kind: ExprKind) {
    match kind {
        // Leaf variants — no children to extract.
        ExprKind::IntLiteral(..)
        | ExprKind::FloatLiteral(..)
        | ExprKind::BoolLiteral(_)
        | ExprKind::StringLiteral(_)
        ...
        ExprKind::InterpolatedString(parts) => {
            for part in parts {
                if let StringPart::Expr(expr) = part {
                    take_boxed_expr(stack, expr);
                }
            }
        }

        ExprKind::Binary(bin) => {
            let BinaryExpr { left, right, .. } = *bin;
            take_expr(stack, left);
            take_expr(stack, right);
        }

        ExprKind::Unary(un) => {
            let UnaryExpr { operand, .. } = *un;
            take_expr(stack, operand);
        }

        ExprKind::Call(call) => {
            let CallExpr { callee, args } = *call;
            take_expr(stack, callee);
            for arg in args {
                take_expr(stack, arg);
            }
        }
        ...
        ExprKind::Assign(assign) => {
            ...
            drain_assign_target(stack, target);
            take_expr(stack, value);
        }

        ExprKind::CompoundAssign(ca) => {
            let CompoundAssignExpr { target, value, .. } = *ca;
            drain_assign_target(stack, target);
            take_expr(stack, value);
        }
        ...
        ExprKind::Range(range) => {
            ...
            take_expr(stack, start);
            take_expr(stack, end);
        }

        ExprKind::Grouping(inner) => {
            take_boxed_expr(stack, inner);
        }
        ...
        ExprKind::RepeatLiteral { element, .. } => {
            take_boxed_expr(stack, element);
        }

        ExprKind::Index(idx) => {
            let IndexExpr { object, index } = *idx;
            take_expr(stack, object);
            take_expr(stack, index);
        }
        ...
        ExprKind::Match(m) => {
            ...
            take_expr(stack, scrutinee);
            for arm in arms {
                drain_pattern(stack, arm.pattern);
                if let Some(guard) = arm.guard {
                    take_expr(stack, guard);
                }
                take_expr(stack, arm.body);
            }
        }

        ExprKind::NullCoalesce(nc) => {
            let NullCoalesceExpr { value, default } = *nc;
            take_expr(stack, value);
            take_expr(stack, default);
        }
        ...
        ExprKind::Lambda(lam) => {
            ...
            drain_type_params(stack, type_params);
            for p in params {
                if let Some(te) = p.ty {
                    take_type_expr(stack, te);
                }
        ...
        ExprKind::TypeLiteral(te) => {
            take_boxed_type_expr(stack, te);
        }

        ExprKind::StructLiteral(sl) => {
            let StructLiteralExpr {
                type_args, fields, ..
            } = *sl;
            for ta in type_args {
                take_type_expr(stack, ta);
            }
            for f in fields {
                take_expr(stack, f.value);
            }
        ...
        ExprKind::FieldAccess(fa) => {
            let FieldAccessExpr { object, .. } = *fa;
            take_expr(stack, object);
        }

        ExprKind::OptionalChain(oc) => {
            let OptionalChainExpr { object, .. } = *oc;
            take_expr(stack, object);
        }
        ...
        ExprKind::When(when) => {
            let WhenExpr { arms, .. } = *when;
            for arm in arms {
                if let Some(cond) = arm.condition {
                    take_expr(stack, cond);
                }
                take_expr(stack, arm.body);
            }
        }
...
/// Extract all child nodes from a `TypeExprKind`.
fn drain_type_expr_kind(stack: &mut Vec<DropItem>, kind: TypeExprKind) {
    match kind {
        ...
        TypeExprKind::Array(inner) | TypeExprKind::Optional(inner) => {
            take_boxed_type_expr(stack, inner);
        }

        TypeExprKind::Union(types)
        | TypeExprKind::Combination(types)
        | TypeExprKind::Tuple(types) => {
            for t in types {
                take_type_expr(stack, t);
            }
        }
        ...
        TypeExprKind::Function {
            ...
            for p in params {
                take_type_expr(stack, p);
            }
            take_boxed_type_expr(stack, return_type);
        }
        ...
        TypeExprKind::Fallible {
            ...
            take_boxed_type_expr(stack, success_type);
            take_boxed_type_expr(stack, error_type);
        }

        TypeExprKind::Generic { args, .. } => {
            for a in args {
                take_type_expr(stack, a);
            }
        }

        TypeExprKind::FixedArray { element, .. } => {
            take_boxed_type_expr(stack, element);
        }

        TypeExprKind::Structural { fields, methods } => {
            for f in fields {
                take_type_expr(stack, f.ty);
            }
            for m in methods {
                for p in m.params {
                    take_type_expr(stack, p);
                }
                take_type_expr(stack, m.return_type);
            }
        }

        TypeExprKind::QualifiedPath { args, .. } => {
            for a in args {
                take_type_expr(stack, a);
            }
        }
...
/// Drain type parameters, extracting any `TypeExpr`s from constraints.
fn drain_type_params(stack: &mut Vec<DropItem>, type_params: Vec<TypeParam>) {
    for tp in type_params {
        if let Some(constraint) = tp.constraint {
            match constraint {
                TypeConstraint::Interface(ifaces) => {
                    for iface in ifaces {
                        for ta in iface.type_args {
                            take_type_expr(stack, ta);
                        }
                    }
                }
