...
impl InterfaceVtableRegistry {
    ...
    pub(crate) fn get_or_declare<C: VtableCtx>(
        ...
        let class_info = {
            let vir_table = &ctx.analyzed().type_table;
            vir_table
    ...
    pub(crate) fn ensure_compiled<C: VtableCtx>(
        ...
        let iterator_info: IteratorVtableInfo = if ctx
            .analyzed()
            .name_table()
            .well_known
        ...
        for (index, &method_id) in state.method_ids.iter().enumerate() {
            let method = ctx.analyzed().get_method_def(method_id);
...
fn resolve_concrete_type_name_vir<C: VtableCtx>(
    ...
    let type_def_id = vir_table
        .unwrap_class(vir_id)
        .map(|(id, _)| id)
        .or_else(|| vir_table.unwrap_struct(vir_id).map(|(id, _)| id))
        .ok_or_else(|| {
...
///
/// Consolidates `value_to_word` call sites using VIR type lookup.
#[inline]
fn value_to_word_ctx<C: VtableCtx>(
    ...
    let vir_table = &ctx.analyzed().type_table;
    crate::types::vir_conversions::vir_value_to_word(
        builder,
        value,
        ctx.ptr_type(),
        heap_alloc_ref,
...
fn unwrap_sig_vir_types<C: VtableCtx>(
    ...
    let sig_vir = vir_table
        ...
        .expect("INTERNAL: vtable: signature must be in VIR table");
...
pub(crate) fn interface_method_slot_by_type_def_id(
    ...
    methods
        .iter()
        .position(|method_id| {
            let method = analyzed.get_method_def(*method_id);
...
fn resolve_vtable_target<C: VtableCtx>(
    ...
    let interface_method = ctx.analyzed().get_method_def(interface_method_id);
    ...
    let direct_method_result: Option<(MethodInfo, Vec<VirTypeId>, VirTypeId)> = (|| {
        ...
        let sig_from_entity: Option<(Vec<VirTypeId>, VirTypeId)> = ctx
            ...
            .and_then(|m_id| {
                let method = ctx.analyzed().get_method_def(m_id);
...
fn build_union_tag_remap_v(
    ...
    if callee_vir == concrete_vir_ty {
        ...
        for (i, &v) in callee_variants.iter().enumerate() {
            if matches!(vir_table.get(v), VirType::Param { .. }) {
