...
use vole_vir::{VirTypeMappingEntry, VirTypeMappingKind};
...
impl Cg<'_, '_, '_> {
    ...
    pub(crate) fn native_call_result(
        ...
        if let NativeType::Struct { field_count } = &native_func.signature.return_type {
            ...
            return Ok(self.compiled_with_ty(results[0], self.ptr_type(), type_id));
        }
    ...
    pub(crate) fn resolve_intrinsic_key_for_monomorph(
        &self,
        callee_name: &str,
        type_mappings: &[VirTypeMappingEntry],
    ...
    pub(super) fn try_call_functional_interface(
        ...
        let Some(method_id) = self.analyzed().is_functional_interface(iface_type_def_id) else {
            return Ok(None);
        };
