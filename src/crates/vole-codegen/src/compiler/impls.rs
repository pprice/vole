...
impl Compiler<'_> {
    ...
    /// Compile methods for a module record (uses module interner)
    pub(super) fn compile_module_record_methods(
        &mut self,
        record: &RecordDecl,
        module_interner: &Interner,
        module_path: &str,
        ...
        let Some(metadata) = metadata else {
            tracing::warn!(type_name = %type_name_str, "Could not find metadata for module record");
            return Ok(());
        };
