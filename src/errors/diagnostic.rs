...
pub fn get_line_from_source(source: &str, line_number: u32) -> Option<String> {
    ...
    source.lines().nth((line_number - 1) as usize).map(|s| s.to_string())
}
...
mod tests {
    ...
    fn get_line_from_source_works() {
        let source = "line one\nline two\nline three";
        assert_eq!(get_line_from_source(source, 1), Some("line one".to_string()));
        assert_eq!(get_line_from_source(source, 2), Some("line two".to_string()));
        assert_eq!(get_line_from_source(source, 3), Some("line three".to_string()));
    ...
    fn diagnostic_builder_creates_diagnostic() {
        ...
        let diag = builder.error(&SEMA_TYPE_MISMATCH, span, "expected i64, found bool".to_string());
