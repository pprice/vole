...
impl Compiler<'_> {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // Nested generic call discovery
    // ═══════════════════════════════════════════════════════════════════════

    /// Discover nested generic calls in a function body and add them to late_monomorph_cache.
    /// Returns the number of new instances discovered.
    fn discover_nested_generic_calls(
        &mut self,
        func_body: &FuncBody,
        substitutions: &HashMap<NameId, TypeId>,
        generic_func_infos: &HashMap<NameId, &vole_sema::entity_defs::GenericFuncInfo>,
    ) -> usize {
        use vole_frontend::ExprKind;
        use vole_sema::generic::{MonomorphKey, MonomorphInstance};
        use vole_sema::types::FunctionType;

        let mut new_count = 0;

        // Helper to walk expressions and find call target symbols
        fn find_calls_in_expr(expr: &Expr, calls: &mut Vec<Symbol>) {
            match &expr.kind {
                ExprKind::Call(call) => {
                    if let ExprKind::Identifier(sym) = &call.callee.kind {
                        calls.push(*sym);
                    }
                    // Also check callee and args for nested calls
                    find_calls_in_expr(&call.callee, calls);
                    for arg in &call.args {
                        find_calls_in_expr(arg, calls);
                    }
                }
                ExprKind::Binary(bin) => {
                    find_calls_in_expr(&bin.left, calls);
                    find_calls_in_expr(&bin.right, calls);
                }
                ExprKind::Unary(un) => {
                    find_calls_in_expr(&un.operand, calls);
                }
                ExprKind::If(if_expr) => {
                    find_calls_in_expr(&if_expr.condition, calls);
                    find_calls_in_expr(&if_expr.then_branch, calls);
                    if let Some(else_branch) = &if_expr.else_branch {
                        find_calls_in_expr(else_branch, calls);
                    }
                }
                ...
                ExprKind::FieldAccess(fa) => {
                    find_calls_in_expr(&fa.object, calls);
                }
                ExprKind::MethodCall(mc) => {
                    find_calls_in_expr(&mc.object, calls);
                    for arg in &mc.args {
                        find_calls_in_expr(arg, calls);
                    }
                }
                ExprKind::Index(idx) => {
                    find_calls_in_expr(&idx.object, calls);
                    find_calls_in_expr(&idx.index, calls);
                }
                ExprKind::StructLiteral(sl) => {
                    for field in &sl.fields {
                        find_calls_in_expr(&field.value, calls);
                    }
                }
        ...
        fn find_calls_in_stmt(stmt: &Stmt, calls: &mut Vec<Symbol>) {
            match stmt {
                Stmt::Expr(e) => find_calls_in_expr(&e.expr, calls),
                Stmt::Let(let_stmt) => {
                    if let LetInit::Expr(expr) = &let_stmt.init {
                        find_calls_in_expr(expr, calls);
                    }
                ...
                Stmt::While(w) => {
                    find_calls_in_expr(&w.condition, calls);
                    for s in &w.body.stmts {
                        find_calls_in_stmt(s, calls);
                    }
                }
                Stmt::For(f) => {
                    find_calls_in_expr(&f.iterable, calls);
                    for s in &f.body.stmts {
                        find_calls_in_stmt(s, calls);
                    }
                }
                Stmt::If(if_stmt) => {
                    find_calls_in_expr(&if_stmt.condition, calls);
        ...
        // Collect all calls in the function body
        ...
        // Process each call to check if it's a generic function
        let main_module = self.query().main_module();
        let interner = &self.analyzed.interner;

        for callee_sym in calls {
            // Look up the function name
            let callee_name_id = self.query().try_function_name_id(main_module, callee_sym);
            let Some(callee_name_id) = callee_name_id else {
                continue;
            };

            // Check if it's a generic function
            let Some(generic_info) = generic_func_infos.get(&callee_name_id) else {
                continue;
            };

            // Build type_keys by substituting generic param types
            ...
            // If we couldn't resolve all type params, skip
            if type_keys.len() != generic_info.type_params.len() {
                continue;
            }

            let key = MonomorphKey::new(callee_name_id, type_keys.clone());

            // Check if already in original cache
            if self.analyzed.entity_registry().monomorph_cache.contains(&key) {
                continue;
            }

            // Check if already in late cache
            if self.state.late_monomorph_cache.borrow().contains_key(&key) {
                continue;
            }

            // Create new instance
            let id = self.state.late_monomorph_cache.borrow().len() as u32 + 10000; // Offset to avoid collision
            let module_id = self.analyzed.name_table().module_of(callee_name_id);
            let base_str = self
                .analyzed
                .name_table()
                .last_segment_str(callee_name_id)
                .unwrap_or_else(|| interner.resolve(callee_sym).to_string());
            let mangled_name = {
                let mut table = self.analyzed.name_table_mut();
                use vole_identity::Namer;
                let mut namer = Namer::new(&mut table, interner);
                namer.monomorph_str(module_id, &base_str, id)
            };

            // Create concrete function type by substituting
            let subs_hashbrown: FxHashMap<_, _> =
                substitutions.iter().map(|(&k, &v)| (k, v)).collect();
            let (concrete_param_ids, concrete_return_id) = {
                let mut arena = self.analyzed.type_arena_mut();
            ...
            tracing::debug!(
                callee = %interner.resolve(callee_sym),
                "discovered nested generic call, creating late monomorph instance"
            );

            self.state.late_monomorph_cache.borrow_mut().insert(key, instance);
            new_count += 1;
        }
    ...
    /// Expand monomorph instances by discovering nested generic calls.
    /// Iterates until no new instances are found.
    fn expand_monomorph_instances(&mut self, program: &Program) {
        use vole_sema::entity_defs::GenericFuncInfo;

        // Build map of generic function names to their info
        let generic_func_infos: HashMap<NameId, &GenericFuncInfo> = self
            .analyzed
            .entity_registry()
            .iter_functions()
            .filter_map(|func_def| {
                func_def
                    .generic_info
                    .as_ref()
                    .map(|info| (func_def.full_name_id, info))
            })
            .collect();
        ...
        loop {
            ...
            // Process instances from original cache
            let original_instances: Vec<_> = self
                .analyzed
                .entity_registry()
                .monomorph_cache
                .collect_instances();
            ...
            for instance in &original_instances {
                ...
                if let Some(func) = generic_func_asts.get(&instance.original_name) {
                    new_total +=
                        self.discover_nested_generic_calls(&func.body, &instance.substitutions, &generic_func_infos);
                }
            }

            // Process instances from late cache
            let late_instances: Vec<_> = self.state.late_monomorph_cache.borrow().values().cloned().collect();
            for instance in &late_instances {
                if self.is_external_func(instance.original_name) {
                    continue;
                }
                if let Some(func) = generic_func_asts.get(&instance.original_name) {
            ...
            tracing::debug!(
                ...
                "discovered nested generic calls, iterating"
            );
        }
    }
    ...
    fn declare_all_monomorphized_instances(&mut self, program: &Program) -> Result<(), String> {
        // First, expand instances to include nested generic calls
        self.expand_monomorph_instances(program);

        self.declare_monomorphized_instances()?;
        self.declare_late_monomorphized_instances()?;
        self.declare_class_method_monomorphized_instances()?;
        self.declare_static_method_monomorphized_instances()?;
        Ok(())
    }

    /// Declare late-discovered monomorphized function instances
    fn declare_late_monomorphized_instances(&mut self) -> Result<(), String> {
        let instances: Vec<_> = self.state.late_monomorph_cache.borrow().values().cloned().collect();

        tracing::debug!(
            instance_count = instances.len(),
            "declaring late monomorphized function instances"
        );
    ...
    fn compile_all_monomorphized_instances(&mut self, program: &Program) -> Result<(), String> {
        ...
        self.compile_late_monomorphized_instances(program)?;
        self.compile_class_method_monomorphized_instances(program)?;
        self.compile_static_method_monomorphized_instances(program)?;
        Ok(())
    }

    /// Compile late-discovered monomorphized function instances
    fn compile_late_monomorphized_instances(&mut self, program: &Program) -> Result<(), String> {
        // Build a map of generic function names to their ASTs
        let generic_func_asts: HashMap<NameId, &FuncDecl> = program
            .declarations
            .iter()
            .filter_map(|decl| {
                if let Decl::Function(func) = decl {
                    if !func.type_params.is_empty() {
                        let name_id = self.query().function_name_id(
        ...
        tracing::debug!(
            instance_count = instances.len(),
            "compiling late monomorphized function instances"
        );
        ...
        for instance in instances {
            ...
            // Find the original generic function AST
            let func = generic_func_asts.get(&instance.original_name).ok_or_else(|| {
                format!(
                    "generic function not found for late monomorph: {:?}",
